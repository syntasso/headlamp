...
import { shouldRetryKubeRequest } from './retry';

describe('shouldRetryKubeRequest', () => {
  ...
  it('keeps the default retry limit for other errors', () => {
    expect(shouldRetryKubeRequest(0, new ApiError('Unavailable', { status: 503 }))).toBe(true);
    expect(shouldRetryKubeRequest(2, new Error('Network error'))).toBe(true);
    expect(shouldRetryKubeRequest(3, new ApiError('Unavailable', { status: 503 }))).toBe(false);
  });
});
